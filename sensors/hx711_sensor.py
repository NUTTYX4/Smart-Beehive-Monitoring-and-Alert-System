...
class HX711Sensor:
    ...
    def calibrate(
        ...
        offset = float(self._hx.get_offset()) if hasattr(self._hx, "get_offset") else baseline_med
    ...
    def apply_saved_calibration(self) -> CalibrationData:
        ...
        if self._hx is not None:
            try:
                ...
                if getattr(cal, "offset", 0.0) != 0.0:
                    if hasattr(self._hx, "set_offset"):
                        self._hx.set_offset(cal.offset)
                    else:
                        self._hx.OFFSET = cal.offset
                    logger.info("Applied saved tare zero offset: %.2f (preserving absolute weight across reboot)", cal.offset)
            except Exception as exc:  # noqa: BLE001
